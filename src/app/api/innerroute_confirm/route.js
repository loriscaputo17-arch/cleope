...
export async function POST(req) {
  try {
    ...
    /* ================= EVENT DATA ================= */
    const EVENT_DATE_START = new Date(Date.UTC(2026, 1, 7, 19, 0)); // 20:00 CET
    const EVENT_DATE_END = new Date(Date.UTC(2026, 1, 8, 2, 0));   // 03:00 CET

    const summary = "THE INNER ROUTE – Part I";
    const description =
      "You are on the guest list for The Inner Route – Part I. Give your name at the entrance.";
    const location = "Shinto Milano, Corso Sempione 30";
    const uid = `innerroute-${to.replace(/[^a-zA-Z0-9]/g, "")}@cleope.events`;
    ...
    const icsContent = buildICS({
      ...
      description,
      location,
      url: `${baseUrl}/the-inner-route/part-one`,
    });
    ...
    const html = `
      <div style="font-family:Helvetica,Arial,sans-serif;background:#000;color:#fff;padding:48px;text-align:center;border:1px solid #222;">
        ...
        <p style="color:#777;font-size:12px;margin-bottom:36px;letter-spacing:2px;text-transform:uppercase;">
          Part I · Shinto Milano · Corso Sempione, 30
        </p>
        ...
        <div style="background:#0e0e0e;border:1px solid #222;padding:22px;margin:32px auto;max-width:440px;text-align:left;">
          ...
          <ul style="list-style:none;padding:0;margin:0;font-size:13px;line-height:1.7;color:#bbb;">
            ...
            <li><strong>Entry before 00:30:</strong> €15 (drink included)</li>
            <li><strong>Entry after 00:30:</strong> €20 (drink included)</li>
          </ul>
        </div>
    ...
    await transporter.sendMail({
      ...
      to,
      subject: "You are on the guest list — The Inner Route",
      html,
      attachments: [
        {
          filename: "the-inner-route.ics",
          content: icsContent,
